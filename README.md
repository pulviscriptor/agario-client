...
## Instructions ##
You need [Node.js](https://nodejs.org/) (i used **v0.10.21** so anything higher should work). Create some folder and put there `agario-client.js` and `example.js`. Also you need [buffer-dataview](https://www.npmjs.com/package/buffer-dataview) lib. You can install it using `npm install buffer-dataview`. Then `node example.js` should work after you set server in it (see instructions inside `example.js`)
